...
class TestDataset(data.Dataset):
    def __init__(self, test_dir, test_file, in_ch, out_ch):
        super().__init__()
        ...
        # 该步之前已经确定两个参数test_dir, test_file必定存在其一
        self.test_files = []
    ...
    def __getitem__(self, index):
        if len(self.test_files) != 0:
        ...
        elif (self.test_file is not None) and (os.path.isfile(self.test_file)):
            filename = os.path.basename(self.test_file)
            x = cv2.imread(self.test_file, 1).astype(np.float32)
            ...
            print("[Error] Invalid test dataset. Test_dir is empty or test_file not exist.")
            exit()
